                ...
                <div class="row">
                    <div class="col-lg-12">
						...
						<div class="navbar-right col-lg-10">
							<a href="<?=site_url()?>/c_kategori/add_page" class="navbar-right btn btn-info ">Tambah Kategori</a>
							
							<div class="navbar-right col-sm-4">
								<div class="input-group">
									<input type="text" class="form-control col-sm-8" name="title1" id="title1" placeholder="Cari Kategori">
									<div class="input-group-btn">
										<a title="Cari" id="btn_ok1" name="btn_ok1" class="btn btn-default"><i class="fa fa-search fa-fw"></i></a>
										<a title="Refresh" class="btn btn-default" href="<?=site_url()?>/c_Kategori"><i class="glyphicon glyphicon-refresh icon-refresh"></i></a>
									</div>
								</div><!-- /input-group -->
...
function operateFormatter1(value, row, index) {
	return [
		'<a title="Lihat Detil" href="<?=site_url()?>/c_kategori/view_page/'+row['id']+'" class="btn btn-primary">',
			'<i class="fa fa-eye fa-fw"></i>',
		'</a> ',
		'<a title="Ubah" href="<?=site_url()?>/c_kategori/edit_page/'+row['id']+'" class="btn btn-success">',
...
window.operateEvents = {
	'click .remove': function (e, value, row, index) {
		...
		if (answer==true){
			$.ajax({
			type: "post",
			url: url+"c_Kategori/delete/"+row['id'],
			success: function (){
...
function tablePegawai2() {
	$('#table-javascript1').bootstrapTable({
		method: 'post',
		url: '<?=site_url()?>/json/json_kategori',
		cache: true,
		striped: true,
		pagination: true,
		pageSize: 10,
...
function tablePegawai3() {
	var url_string = "<?=site_url()?>/json/json_kategori_search";
